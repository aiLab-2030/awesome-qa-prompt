...
Awesome QA Prompt 是面向软件测试与质量保证的 **Prompt Baseline**：以可复制使用的提示词为主，而不是面向 AI 工具的技能安装包。项目持续维护中英文模块、标准/精简版本，以及部分框架与平台变体。

### 🌟 项目价值

通过提供结构化的提示词模板，帮助测试工程师：

- **🚀 提升测试效率**：用结构化提示词快速产出可执行的测试方案与用例初稿
- **🎯 确保测试覆盖**：系统化地覆盖功能、性能、安全等各个维度，减少遗漏关键测试点
- **📋 标准化流程**：统一测试文档格式和测试执行思路，便于团队协作
- **💡 知识传承**：沉淀可复用的提示词与实践，帮助新人更快上手

### 🎨 设计理念

- **专业导向**：基于常见测试实践与可落地的工作方式
- **实用至上**：优先产出最小可执行结果，而不是超长万能模板
- **易于使用**：复制粘贴即可使用，默认入口清晰
- **持续改进**：根据反馈持续修正断链、误导性表述和不可用内容
...
- **文件位置**：`testing-types/zh/requirements-analysis/`
...
- **文件位置**：`testing-types/zh/test-case-writing/`
...
1. **选择提示词**
   - 进入 `testing-types/zh/<模块>/`（英文项目用 `testing-types/en/<模块>/`）
   - 默认打开 `Standard-version/` 下对应 Prompt；需要时再选 Lite、框架版或平台场景版
   - 可选择标准版本或 TOP 5 框架版本（ROSES、LangGPT、ICIO、CRISPE、RISE）

2. **准备输入材料**
   - 补充真实项目上下文：范围、环境、限制、风险、期望产出
   - 例如：需求分析需要提供需求文档或 User Story

3. **复制提示词**
   - 若 Prompt 含分隔线 `---`，从首个分隔线后复制；如果文件没有分隔线，则复制全文
   - 将复制内容粘贴到 AI 助手中
   - 将你的输入材料附加到提示词后面

4. **获取输出**
   - AI 助手会根据提示词生成结构化的测试文档
   - 根据需要进行调整和完善；信息不足时应先澄清或标注假设
...
- 使用时请统一按以下方式操作：
  - 选择一个提示词文件
  - 若 Prompt 含分隔线 `---`，从首个分隔线后复制；如果文件没有分隔线，则复制全文到 AI 助手
