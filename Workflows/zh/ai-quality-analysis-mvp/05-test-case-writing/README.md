...
[English](../../../en/ai-quality-analysis-mvp/05-test-case-writing/README.md) | 简体中文
...
1. 锁定同一版本的需求、需求分析、测试策略、策略评审，以及可选技术方案和代码评审。
2. 将同一输入分别交给四份角色 Prompt；产品和 UI/UX 先作条件参与判断。
3. 检查每条建议是否包含最低八类字段，并确认所有步骤、数据和预期均有输入依据。
4. 将四份角色报告与同一组上游材料交给汇总 Prompt，完成去重、统一编号和追踪矩阵。
