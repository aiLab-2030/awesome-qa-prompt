...
[English](../../en/ai-quality-analysis-mvp/README.md) | 简体中文
...
4. **再调用汇总 Prompt**：只传递阶段 README 明确允许的角色报告和 Artifact。阶段 1、2、3、6、7 的汇总只读取角色报告；阶段 4、5、8 还需读取各自阶段明确声明的 Artifact。
...
| 2. 测试策略 | 需求文档、需求分析报告、代码版本或可读取仓库链接 | 交互原型图、原始需求、技术方案 |
| 3. 测试策略评审 | 测试策略正文或可读取位置，以及名称、来源、版本 | 无 |
| 4. 代码评审 | 明确代码版本及对应 Diff、补丁、完整文件或可读取仓库位置 | 推荐需求和需求分析；可选技术方案 |
| 5. 测试用例编写 | 需求、需求分析、测试策略、测试策略评审报告 | 技术方案、代码评审报告 |
